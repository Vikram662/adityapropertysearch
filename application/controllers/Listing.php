...
class Listing extends CI_Controller{
        ...
        $data['view'] = 'app/listing';
         ...
         $this->load->library('pagination');
          if(@$this->session->userdata('city')){
              $data['num']= $this->db->where('city',$this->session->userdata('city'))->count_all_results('property');
          }else{
  ...
  {
    $post = $this->input->post();
       $data = $this->db->get_where('enq',['id'=>$post['otp_id']])->row();
       if($data->otp==$post['otp']){
          $this->db->where('id',$this->session->userdata('otp_id'))->update('enq',['verfy'=>1]);
          $this->session->set_flashdata('msg', 'Query Succesfully Submitted');
        $this->session->unset_userdata('otp_no');
      
        $this->session->set_userdata(['verfied'=>TRUE] );
           ?>
        <script>
          window.history.back();
        </script>
  ...
  public function send()
  {
       ...
       $post['date'] = date('Y-m-d H:i:s');
        ...
        $otpid = $this->db->insert_id();
          $this->session->set_flashdata('msg', 'One Time Password(OTP) Send Your Mobile No');
          $this->session->set_userdata('otp_no',$otpid);
          $msg = "Your Verification code Is ".$post['otp'];
          $this->load->library('sms');
          $this->sms->sendsms($msg,$post['phone']);
         ?>
        <script>
          window.history.back();
        </script>
